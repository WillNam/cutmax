...
This is the **cutmax-specific** guide to HyperFrames. It explains when
cutmax pipelines should choose HyperFrames over Remotion, how cutmax
...
## When cutmax should pick HyperFrames (vs Remotion vs FFmpeg)

cutmax separates two concepts:
...
| Synthetic UI / fake terminal / fake browser demo | Either — depends on existing coverage | cutmax already ships Remotion `TerminalScene` (see `synthetic-screen-recording` Layer 3). For UI chrome beyond terminal, HyperFrames HTML is easier. |
...
cutmax artifacts into HyperFrames project files:

| cutmax artifact field | HyperFrames target |
|---|---|
...
`STORYBOARD.md` as step-by-step workspace files. cutmax does **not**
...
cutmax playbooks currently translate into Remotion `themeConfig`
...
- ❌ Running `hyperframes init` from the cutmax orchestrator. `init`
