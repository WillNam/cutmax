...
cutmax pull requests. It is a review framework, not a closed checklist.
Use it to structure the review, then keep looking for issues that are specific
to the PR in front of you.

cutmax is an agent-orchestrated video production system. Many PRs affect
...
- Does this PR move cutmax in the right direction?
...
Check whether the PR solves a real cutmax problem.
...
cutmax works with video, audio, image generation, and local models. Small
...
## cutmax Architecture Checks

### Agent-First Architecture

cutmax's control plane is the agent following markdown skills and YAML
...
cutmax uses checkpoints for resume, human approval, audit trails, and
stage gating.
...
cutmax can compose with Remotion, HyperFrames, or FFmpeg. Runtime choices
...
cutmax should not silently spend money or imply a paid provider is free.
...
- Is this provider useful for cutmax workflows?
...
- Is the provider internationally known, nationally or regionally important,
  or otherwise clearly valuable to cutmax users?
...
Provider viability matters. cutmax should not become a grab bag of
