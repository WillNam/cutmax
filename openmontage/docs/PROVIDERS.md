# cutmax Provider Guide

Everything you need to know about every provider in cutmax — setup instructions, pricing, free tiers, and what each unlocks.
...
cutmax now uses those published rates in the Grok tool estimators.
...
cutmax requires `custom_price_cny_per_million_tokens` before presenting a
...
cutmax conservatively reports the standard pay-as-you-go amount in cost
...
routes. cutmax validates each model's real schema instead of treating task
...
- Official Kling is a paid remote API. cutmax uses conservative cost estimates and includes high-cost factors such as Omni references, series output, 4k mode, and native sound.
...
- Video Omni and Image Omni can pass official `element_id` references through `element_list`; Elements remain an internal Kling Official helper, not a standalone cutmax capability.
...
- `callback_url` is passed through and recorded when supplied, but cutmax still polls tasks by default.
- `kling_tts` requires an explicit `voice_id`; cutmax does not guess a default official voice.
...
- Official Kling audio effects and video effects are documented but intentionally not registered as cutmax tools yet, because current pipelines do not have a stable sound-effects or video-effects capability slot for them.
...
cutmax uses the new-console API key flow:
...
Doubao Speech 2.0 is billed by character package or usage in Volcengine. cutmax estimates cost from text length and prefers provider-returned usage metadata when available.
...
cutmax uses the **Fast Transcription** REST endpoint, which accepts a local
...
For files longer than ~2 hours or bulk jobs, use Azure Batch Transcription instead (not wired into cutmax).
...
allowance). cutmax estimates cost from the transcribed audio duration. See
...
cutmax uses the synchronous REST v1 endpoint with an SSML body — no token
...
costs about $0.015. cutmax estimates cost from character count. See
[Azure AI Speech pricing](https://azure.microsoft.com/pricing/details/cognitive-services/speech-services/) for current rates.
...
> **Multi-model production API.** cutmax supports current Runway-native
...
The CLI is consumed as `npx hyperframes`. Do not use `npx @hyperframes/cli`; that package name is not the cutmax runtime path.
...
HyperFrames workspaces live under `projects/<project-name>/hyperframes/`. Final videos still follow the normal cutmax convention: `projects/<project-name>/renders/final.mp4`.
...
text encoder, video VAE, and audio VAE. cutmax exposes the official
