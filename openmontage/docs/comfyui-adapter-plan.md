# ComfyUI Provider Adapter for cutmax
...
cutmax's local GPU tools (`wan_video`, `hunyuan_video`, `cogvideo_video`,
...
A ComfyUI adapter gives cutmax access to any model ComfyUI supports,
...
**Selector integration:** no dedicated `music_selector` exists in cutmax
...
URL when cutmax knows the canonical source. Agents should surface that
...
using cutmax's 7-dimension scoring:
...
cutmax should treat those as custom workflow profiles until a blessed
...
### Future (add models to ComfyUI, no code changes to cutmax)
