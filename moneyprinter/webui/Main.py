...
# 避免依赖中的同名 app 包遮蔽 cutmax-moneyprinter 自己的 app 包。
...
st.set_page_config(
    page_title="cutmax-moneyprinter",
    page_icon="🤖",
    layout="wide",
    initial_sidebar_state="auto",
    menu_items={
        "Report a bug": "https://github.com/WillNam/cutmax/issues",
        "About": "# cutmax-moneyprinter\nSimply provide a topic or keyword for a video, and it will "
...
# cutmax-moneyprinter 是面向终端用户的本地工具，这些入口会造成顶部大块空白，
...
def _render_brand(available_update: str | None = None):
    ...
    st.markdown(
        f"""
        <h1 class="mpt-brand">
            <span class="mpt-brand__name">cutmax-moneyprinter</span>
            <a class="mpt-brand__version"
               href="https://github.com/WillNam/cutmax"
               target="_blank"
               rel="noopener noreferrer"
               aria-label="Open cutmax-moneyprinter on GitHub"
...
def _render_local_script_generation(params):
    """保留 cutmax-moneyprinter 原有的本地 LLM 脚本生成路径。"""
    if not st.button(
