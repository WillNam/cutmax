#!/usr/bin/env python3
"""Cross-platform installation and video generation for the cutmax-moneyprinter Skill."""
...
PROJECT_ARCHIVE_URL = (
    "https://github.com/WillNam/cutmax/archive/refs/heads/main.zip"
)
DEFAULT_ROOT = Path.home() / "cutmax-moneyprinter"
...
RECOMMENDED_LLM_PROVIDERS = {
    ...
    "volcengine": (
        ...
        "https://www.volcengine.com/activity/ai618?utm_source=cutmax-moneyprinter",
    ),
...
def log(message: str) -> None:
    ...
    print(f"[cutmax-moneyprinter] {message}", flush=True)


def parse_args(argv: list[str] | None = None) -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Install cutmax-moneyprinter and generate a final video from a topic."
    )
    ...
    parser.add_argument(
        ...
        help=f"cutmax-moneyprinter installation directory (default: {DEFAULT_ROOT})",
    )
    parser.add_argument(
        "cli_args",
        nargs=argparse.REMAINDER,
        help="additional cutmax-moneyprinter CLI arguments placed after --",
    )
...
def ensure_project(root: Path) -> None:
    ...
    with tempfile.TemporaryDirectory(prefix="mpt-install-") as temp_dir_value:
        ...
        archive_path = temp_dir / "cutmax-moneyprinter.zip"
        request = urllib.request.Request(
            PROJECT_ARCHIVE_URL,
            headers={"User-Agent": "cutmax-moneyprinter-Agent-Skill"},
        )
        ...
        if len(candidates) != 1:
            raise SkillError("download completed but no valid cutmax-moneyprinter project was found")
        if root.exists():
            root.rmdir()
...
def _validate_pexels_key(api_key: str) -> str:
    ...
    request = urllib.request.Request(
        ...
        headers={
            ...
            "User-Agent": "cutmax-moneyprinter-Agent-Skill",
        },
    )
    try:
