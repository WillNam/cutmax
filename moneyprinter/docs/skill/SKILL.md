...
description: Use this skill whenever the user wants to create a finished video from a topic, title, idea, prompt, or script with cutmax-moneyprinter. This includes short-form, voice-over, educational, marketing, social-media, and stock-footage videos. Also use it when the user mentions cutmax-moneyprinter, provides this Skill URL, asks an AI agent to install or configure cutmax-moneyprinter, needs missing API keys identified, wants a failed generation repaired, or wants a generated MP4 located and delivered. Use this skill when the expected outcome is a final video file, not setup instructions.
...
metadata:
  author: "WillNam@hotmail.com"
  version: "1.3.2"
  upstream: "https://github.com/WillNam/cutmax"
---

# cutmax-moneyprinter Video Generation
...
Unless the user requests otherwise, generate one Chinese `9:16` portrait video with Pexels footage, the default Chinese Edge TTS voice, subtitles, and background music. Install cutmax-moneyprinter under the user's home directory.
...
```text
https://raw.githubusercontent.com/WillNam/cutmax/main/docs/skill/mpt_agent.py
```
...
```text
~/cutmax-moneyprinter/.agent-logs/moneyprinterturbo-video/latest-result.json
```
...
- Use uv and the cutmax-moneyprinter CLI only.
