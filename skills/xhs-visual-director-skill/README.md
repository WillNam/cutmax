...
这是一个用于规划并生成小红书图文的 Agent Skill。它不是普通文案助手，而是一个“视觉导演”：先判断内容任务，再选择适合的视觉风格，先生成 1 张视觉确认图，确认后直接生成整套最终图片，并附带图文结构、提示词记录、发布文案和自检清单。
...
1. 选题判断
2. 信息充足度判断
3. 信息不足时，先用苏格拉底式提问法提出 10-20 个问题
4. 回答摘要与生成假设
5. 核心观点
6. 风格判断报告
7. 三套风格方案
...
1. 输入主题或草稿。
2. 如果信息很少，让 Skill 先用苏格拉底式提问法问 10-20 个问题。
3. 回答问题后，让 Skill 总结“回答摘要与生成假设”。
4. 再让 Skill 输出风格判断报告，不急着生成页面。
5. 从三套风格方案里选择一个方向。
6. 生成统一视觉母版和 1 张视觉确认图。
7. 视觉确认通过后，直接生成 8 页最终图片。
8. 根据 `templates/visual_review_checklist.md` 审查比例、可读性和高级感。
9. 最后生成标题、正文、标签和置顶评论。
...
1. 先生成 1 张封面或关键页视觉确认图。
2. 你确认风格、构图、色彩、信息密度。
3. 确认后再生成整套 6-8 页最终图。
4. 输出每张图片的本地路径和比例检查结果。
...
复杂商业、产品、课程、个人品牌或多素材项目会扩展到 15-20 个问题。
...
[![Star History Chart](https://api.star-history.com/svg?repos=WillNam/cutmax&type=Date)](https://www.star-history.com/#WillNam/cutmax&Date)
