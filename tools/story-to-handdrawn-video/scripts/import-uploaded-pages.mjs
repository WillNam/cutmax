...
for (let index = 0; index < inputs.length; index += 1) {
  ...
  // layout=full should fill the canvas like page-flip masters do. The old
  // safe_uncropped path parks art in a bottom band (LayerWipe top:382), which
  // makes portrait subjects look tiny on full-page uploads.
  const useFullPage = paperFlip || layout === 'full';
  scenes.push({
    id,
    duration_sec: pageDuration,
    text: '',
    visual: `上传图片 ${id}`,
    shot: useFullPage ? 'full_uploaded_page' : 'safe_uncropped_uploaded_page',
    // Page curls / full-page uploads use the untouched vertical master. This
    // preserves every handwritten line and avoids auto-crop errors on long
    // captions; it also keeps subjects large in the frame.
    layers: useFullPage
      ? ['color']
      : hasCaption
        ? ['text', 'bw_full', 'color']
    ...
    caption_box: null,
    assets: {
      text_image: !useFullPage && hasCaption ? asset(textName) : null,
      bw: useFullPage ? null : asset(bwName),
      detail: null,
      color: asset(useFullPage ? masterName : colorName),
    },
  });
  manifestPages.push({
    id,
